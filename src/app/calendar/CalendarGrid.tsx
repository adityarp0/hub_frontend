    ...
    <main className="p-6 max-w-5xl mx-auto">
      <div className="mb-4">
        ...
        <p className="text-sm text-gray-500">
          Due dates are pulled live from your Todos
        </p>
      </div>
      ...
      <div className="grid grid-cols-7 border rounded-lg overflow-hidden dark:border-gray-700">
        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
          <div
            key={d}
            className="text-xs text-center py-2 bg-gray-50 dark:bg-gray-800 text-gray-500"
          >
            {d}
          </div>
        ))}
        ...
        {Array.from({ length: firstDayOfWeek }).map((_, i) => (
          <div
            key={`blank-${i}`}
            className="min-h-[78px] border-t border-l bg-gray-50 dark:bg-gray-800 dark:border-gray-700"
          />
        ))}
        ...
        {cells.map(({ day, dateKey }) => {
          ...
          return (
            <div
              key={dateKey}
              className={`min-h-[78px] border-t border-l p-1 dark:border-gray-700 ${
            ...
            >
              ...
              {dayTodos.slice(0, 2).map((task) => (
                <button
                  key={task.id}
                  onClick={goToTodo}
                  className={`block w-full text-left text-[10px] rounded px-1 py-0.5 mt-0.5 truncate ${
      ...
      {tasks.filter((t) => t.dueDate).length === 0 && (
        <div className="mt-4 border border-dashed rounded-xl p-6 text-center text-sm text-gray-400 dark:border-gray-700">
          No tasks have due dates yet — add one from the Todos page and
